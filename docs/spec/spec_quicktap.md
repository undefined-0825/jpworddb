...
ランダムにシャッフルされたタイルを正しい順番でタップし、語句を完成させる。
...
- 不正解時：
  ...
  - 同じ問題の入力をリセットして再挑戦
- 正誤画像は難易度ごとに切り替える
  - `level=1`（簡単）: `OK_y.png` / `NG_y.png`
  - `level=2`（普通）: `OK_c.png` / `NG_c.png`
  - `level=3`（難しい）: `OK_k.png` / `NG_k.png`
  - `level=4`（超高）: `OK_a.png` / `NG_a.png`
